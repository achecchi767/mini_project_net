...
def load_data_wrapper():
    ...
    training_data = list(zip(training_inputs, training_results))  # ✅ Converted zip() to list(zip())

    validation_inputs = [np.reshape(x, (784, 1)) for x in va_d[0]]
    validation_data = list(zip(validation_inputs, va_d[1]))  # ✅ Converted zip() to list(zip())

    test_inputs = [np.reshape(x, (784, 1)) for x in te_d[0]]
    test_data = list(zip(test_inputs, te_d[1]))  # ✅ Converted zip() to list(zip())
